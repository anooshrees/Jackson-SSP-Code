...
write.table(peaks_vetted,"~/Desktop/SSP/DA_7_12_17/monot_merged_consensus_raw_atac_whitelisted.txt",sep = "\t",col.names = T,row.names = F,quote = F)
write.table(data.frame(sort(GRanges(peaks_vetted[,c("chr","start","end")])))[,c("seqnames","start","end")],"~/Desktop/SSP/DA_7_12_17/monot_merged_consensus_whitelisted.bed",sep = "\t",col.names = F,row.names = F,quote = F)
...
peaks <- dropSeqlevels(GRanges(read.table("~/Desktop/SSP/DA_7_12_17/monot_merged_consensus_raw_atac_whitelisted.txt",sep = "\t",header = T,quote = "")[,c("chr","start","end")]),'chrM')
...
write.table(open.genes,"~/Desktop/SSP/DA_7_12_17/geneset.open.monot.txt",quote = F,sep = "\t",col.names = T,row.names = F)
Hide
...
write.table(monot.expressionSet,"~/Desktop/SSP/DA_7_12_17/monot_rnaseq.txt",sep = "\t",col.names = T,quote = F)
Hide
## Code used to select "expressed genes" from RNAseq data. This has already been carried out in this dataset

library(dplyr)

expression.data <- read.table("~/Desktop/SSP/DA_7_12_17/monot_rnaseq.txt",sep = "\t",header = T,quote = "",stringsAsFactors = F)
...
write.table(expressed.genes,"~/Desktop/SSP/DA_7_12_17/geneset.expressed.monot.txt",quote = F,sep = "\t",col.names = T,row.names = F)
Hide
...
sampid <- sample.info$sampid
samps <- atac.samples

sampinfo <- sampinfo[grep("_PBMC", rownames(sampinfo)), ]

age.group <- relevel(factor(setNames(sampinfo[sampinfo$sampid %in% samps,]$group,sampid)),ref = "HO") # this relevel/ref statement is actually what makes HY-specific peaks to have negative logs. By default, HY peaks would have positive logs.
sex <- relevel(factor(setNames(sampinfo[sampinfo$sampid %in% samps,]$sex,sampid)),ref = "F")
age <- setNames(sampinfo[sampinfo$sampid %in% samps,]$age,sampid)
season <- setNames(sampinfo[sampinfo$sampid %in% samps,]$season,sampid)
batch <- setNames(sampinfo[sampinfo$sampid %in% samps,]$batch,sampid)
cmv.status <- setNames(sampinfo[sampinfo$sampid %in% samps,]$CMV.status,sampid)
cmv.titer <- setNames(sampinfo[sampinfo$sampid %in% samps,]$CMV.titer,sampid)
cell.type <- setNames(sampinfo[sampinfo$sampid %in% samps,]$type,sampid)
depth <- setNames(sampinfo[sampinfo$sampid %in% samps,]$depth,sampid)


atac.tcells.raw <- read.table("~/Desktop/SSP/DA_7_12_17/monot_merged_consensus_raw_atac_whitelisted.txt",sep = "\t",header = T,quote = "") # this corresponds to the vetted peak file you generated above
# atac.samples <- rownames(group.names)
colnames(atac.tcells.raw)[58] <- "VHY402D0_PBMC"

atac.tcells.raw <- atac.tcells.raw[, (colnames(atac.tcells.raw) %in% sampid | colnames(atac.tcells.raw) %in% c("chr", "start", "end"))]
...
group.names <- sampinfo[names(atac.tcells.libsize),] %>% select(sex,group,season) # Use this; accounts for discrepancies between samples seen in atac peaks
                                                                                    # and data that was filtered out of samples due to race, age, health, etc.
group.names$group <- relevel(factor(group.names$group),ref="HY")
group.names$sex <- relevel(factor(group.names$sex),ref="F")

group.names <- data.frame(sex, age.group, season) # All filtered values

atac.tcells.design <- with(group.names,model.matrix(~sex+season+group)) # in our paper, our design was sex+season+group
atac.tcells.design.null <- with(group.names,model.matrix(~sex+season,data = data.frame(atac.tcells.libsize))) # the null design is sex+season (i.e. everything but what we are interested in)
...
sum(atac.tcells.lmtop$adj.P.Val<.05) # 18029
...
write.table(atac.tcells.norm.data,"~/Desktop/SSP/DA_7_12_17/tcells_merged_consensus_voom_atac_whitelisted.txt",quote = F,sep = "\t",row.names = F,col.names = T)
write.table(atac.tcells.adj.data,"~/Desktop/SSP/DA_7_12_17/tcells_merged_consensus_adj_atac_whitelisted.txt",quote = F,sep = "\t",row.names = F,col.names = T)
write.table(cbind(atac.tcells.raw[,c("chr","start","end")],atac.tcells.lmtop),"~/Desktop/SSP/DA_7_12_17/tcells_merged_consensus_atac_results.txt",quote = F,sep = "\t",row.names = F,col.names = T)

# Inputs for visualization scripts

annotated.filter.bed$peakco <- paste(annotated.filter.bed$chr,":",annotated.filter.bed$start,"-",annotated.filter.bed$end,sep = "")
...
write.csv(full.results.age, file="~/Desktop/SSP/DA_7_12_17/full_results_age.csv")
...
rna.raw <- read.table("~/Desktop/SSP/DA_7_12_17/monot_rnaseq.txt",sep = "\t",header = T,quote = "")
aging.rna <- read.table("~/Downloads/aging_rnaseq.txt",sep = "\t",header = T,quote = "")
...
# Now filter for RNA samples only

samp.filter <- apply(cbind(sampinfo$group %in% c("HY","HO"),!sampinfo$race %in% c("A","B"),!sampinfo$ethnicity=="H",!is.na(sampinfo$depth),!grepl("VHY402D[1,2,6,7].*",sampinfo$sampid),!sub("_.*","",sampinfo$sampid) %in% c("HO207","ND1","ND2")),1, all)
# sampid <- rownames(sampinfo[samp.filter,])
samps <- colnames(source.data$RNA)[3:ncol(source.data$RNA)]
sampid <- paste(samps, "_PBMC", sep = "")
...
# Reorganize RNA matrix
# rna.raw$peakco <- paste(rna.raw$chr,":",rna.raw$start,"-",rna.raw$end,sep = "")
 rna.raw$rna.max.val <- apply(rna.raw[, 3:33], 1, max)
 ...
 rna.raw <- test.final
# rna.raw <- rna.raw[, unique(c(grep("PBMC", colnames(rna.raw)), grep("GES15", colnames(rna.raw))))]
# colnames(rna.raw) <- gsub("(_).*","\\1",colnames(rna.raw))
# colnames(rna.raw) <- paste(colnames(rna.raw), "PBMC", sep = "")
# rna.raw <- rna.raw[, sampid]

rna.raw <- aging.rna

rna.raw.mx <- as.matrix(data.frame(rna.raw[,which(!colnames(rna.raw) %in% c("chr","start","end","nhits", "EnsemblID", "rna.max.val"))], row.names = "GeneName"))
# colnames(atac.tcells.raw.mx)[58] <- "VHY402D0_PBMC"
rna.libsize <- colSums(rna.raw.mx)
group.names <- sampinfo[names(rna.libsize),] %>% select(sex,group,season) # Use this; accounts for discrepancies between samples seen in atac peaks
rna.raw.mx <- rna.raw.mx[, which(colnames(rna.raw.mx) %in% rownames(group.names))]
...
rna.design <- with(group.names,model.matrix(~sex+season+group)) # in our paper, our design was sex+season+group
rna.design.null <- with(group.names,model.matrix(~sex+season,data = data.frame(rna.libsize))) # the null design is sex+season (i.e. everything but what we are interested in)
...
sum(rna.lmtop$adj.P.Val<.05) # 117
...
rna.adj.data$GeneName <- row.names(rna.adj.data)

write.table(rna.norm.data,"~/Desktop/SSP/DA_7_12_17/merged_consensus_voom_rna_whitelisted.txt",quote = F,sep = "\t",row.names = F,col.names = T)
write.table(rna.adj.data,"~/Desktop/SSP/DA_7_12_17/merged_consensus_adj_rna_whitelisted.txt",quote = F,sep = "\t",row.names = F,col.names = T)
write.table(cbind(rna.raw[,c("GeneName")],rna.lmtop),"~/Desktop/SSP/DA_7_12_17/merged_consensus_rna_results.txt",quote = F,sep = "\t",row.names = F,col.names = T)
...
write.table(gene.ensembl[which(gene.ensembl$logFC <0), "EnsemblID"], "~/Desktop/SSP/DA_7_12_17/rna_diff_neg.txt", quote = F,sep = "\t",row.names = F,col.names = T)
write.table(gene.ensembl[which(gene.ensembl$logFC >0), "EnsemblID"], "~/Desktop/SSP/DA_7_12_17/rna_diff_pos.txt", quote = F,sep = "\t",row.names = F,col.names = T)
...
atac.homer.tcells <- read.table("~/Desktop/SSP/DA_7_12_17/monot_merged_consensus_raw_atac_whitelisted_annotated.txt",sep = "\t",header = T,quote = "")
...
atac.chromhmm.tcells <- mclapply(dir("~/Desktop/SSP/DA_7_12_17/",pattern = "*chromHMM_*",full.names = T),function(f) {
