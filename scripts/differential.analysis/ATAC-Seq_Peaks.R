...
maxcts.pbmc <- data.frame(maxcts=apply(peaks_vetted[,rownames(sampinfo) %in% colnames(peaks_vetted)],1,max))
...
peaks <- dropSeqlevels(GRanges(read.table("~/Desktop/SSP/monot_merged_consensus_raw_atac_whitelisted.txt",sep = "\t",header = T,quote = "")[,c("chr","start","end")]),'chrM')
...
atac.tcells.raw <- read.table("~/Desktop/SSP/peaks/monot_merged_consensus_raw_atac_whitelisted.txt",sep = "\t",header = T,quote = "") # this corresponds to the vetted peak file you generated above
...
sum(atac.tcells.lmtop$adj.P.Val<.05) # 9087
...
write.table(atac.tcells.norm.data,"~/Desktop/SSP/tcell-peaks2/tcells_merged_consensus_voom_atac_whitelisted.txt",quote = F,sep = "\t",row.names = F,col.names = T)
write.table(atac.tcells.adj.data,"~/Desktop/SSP/tcell-peaks2/tcells_merged_consensus_adj_atac_whitelisted.txt",quote = F,sep = "\t",row.names = F,col.names = T)
write.table(cbind(atac.tcells.raw[,c("chr","start","end")],atac.tcells.lmtop),"~/Desktop/SSP/tcell-peaks2/tcells_merged_consensus_atac_results.txt",quote = F,sep = "\t",row.names = F,col.names = T)
...
rna.raw <- read.table("~/Desktop/SSP/RNA-seq/monot_rnaseq.txt",sep = "\t",header = T,quote = "")
...
do.call(rbind,lapply(split(rna.raw,rna.raw$GeneName),function(chunk) chunk[which.max(chunk$max.val),]))

rna.raw.mx <- as.matrix(data.frame(rna.raw[,which(!colnames(rna.raw) %in% c("EnsemblID", "nhits"))], row.names = "GeneName"))
colnames(atac.tcells.raw.mx)[58] <- "VHY402D0_PBMC"
atac.tcells.raw.mx <- atac.tcells.raw.mx[, which(colnames(atac.tcells.raw.mx) %in% rownames(group.names))]
atac.tcells.libsize <- colSums(atac.tcells.raw.mx)
...
atac.homer.tcells <- read.table("~/Desktop/SSP/tcell-peaks/tcells_merged_consensus_voom_atac_whitelisted_annotated.txt",sep = "\t",header = T,quote = "")
...
atac.chromhmm.tcells <- mclapply(dir("~/Desktop/SSP/tcell-peaks/",pattern = "*chromHMM_*",full.names = T),function(f) {
...
atac.glm.tcells <- read.table("~/Desktop/SSP/tcell-peaks/tcells_merged_consensus_atac_results.txt",sep = "\t",header = T,quote = "")
